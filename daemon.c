...
int main (int argc, char **argv)
{
	...
	*(wverf+4) = (uint32) time(NULL);
	...
	if (!svc_register(transp, MOUNTPROG, MOUNTVERS1, mountprog_3, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (MOUNTPROG, MOUNTVERS1, udp).");
		daemon_exit(0);
	}
	...
	if (!svc_register(transp, MOUNTPROG, MOUNTVERS1, mountprog_3, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (MOUNTPROG, MOUNTVERS1, tcp).");
		daemon_exit(0);
	}
