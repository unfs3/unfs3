 ...
 */
    static void set_ipnet(char *addr, int new)
    {
        ...
        *pos = 0;
        ...
        else if (!inet_aton(net, &cur_host.mask))
            e_error = TRUE;
...
%token < text > PATH % token < text > ID % token < text > WHITE % token < text > IP % token < text > NET % token < text > OLDNET %% exports:
export | export '\n' exports |;
...
hosts:
host | host WHITE hosts | host WHITE;
    ...
    while (item) {
        ...
        while (host) {
            ...
            printf("%s: ip %s mask %s options %i\n",
                   item->path, addrbuf, maskbuf, host->options);
            host = (e_host *) host->next;
        }
        item = (e_item *) item->next;
    }
 ...
 */
static int
find_host(struct in_addr remote, e_item * item)
{
    e_host *host;
 ...
 */
int
exports_options(const char *path, struct svc_req *rqstp)
{
    e_item *list;
 ...
 */
nfsstat3
exports_compat(const char *path, struct svc_req * rqstp)
{
    int prev;
